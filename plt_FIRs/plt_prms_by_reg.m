function info = plt_prms_by_reg(PLACE, info)
%% written by K.Garner, 2020
% for each region, extract the 3 parameter x voxels x condition matrices
% (one for each third of the data)
% plot the response from each voxel along the time dimension, to examine
% the HRF
...
nrow = 2; % one for each condition
sup_title_tex = 'sub-%s-TR-%s-reg-%s';
x=2:2:(order*2);
...
for iReg = 1:nreg
    for iDiv = 1:ndiv
        ...
        if size(tmp,1) > 0
            plot(x, squeeze(tmp(:,:,1))');
            title('tgt left');
            
            subplot(nrow, ncol, (iDiv+ncol))
            plot(x, squeeze(tmp(:,:,2))');
            title('tgt right');
        end
    end
